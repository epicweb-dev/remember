...
/**
 * Remembers and retrieves a value by a given name, or the value generated by an async function `getValue` if the name
 * doesn't exist.
 * The return type is inferred from the return type of `getValue`.
 *
 * @template Value
 * @param {string} name - The name under which to remember the value.
 * @param {() => Promise<Value>} getValue - The async function that generates the value to remember.
 * @returns {Promise<Value>} - The remembered value.
 */
export async function rememberAsync(name, getValue) {
	const thusly = globalThis
	thusly.__remember_epic_web ??= new Map()
	if (!thusly.__remember_epic_web.has(name)) {
		thusly.__remember_epic_web.set(name, await getValue())
	}
	return thusly.__remember_epic_web.get(name)
}
